...
export const Header: React.FC = () => {
  const { user, profile, signOut, isAdmin, hasAdminPanelAccess } = useAuth();
  ...
  const navLinks = [
    ...
    { label: 'Avitel', href: '/products?brand=avitel' },
  ...
  useEffect(() => {
    import('@/services/api').then(({ fetchCategories }) => {
      ...
      <div className="bg-secondary border-b border-border hidden md:block">
        <div className="max-w-7xl mx-auto px-4 py-1.5 flex items-center justify-between text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-4">
            <Link to="/track-order" className="flex items-center gap-1 hover:text-foreground transition-colors">
              <Package size={11} className="text-primary" /> Sifarişi İzlə
            </Link>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center gap-3 h-16">
            ...
            <Link to="/" className="flex items-center shrink-0 group">
              {settings.site_logo ? (
                <img src={settings.site_logo} alt={settings.site_title} className="h-10 object-contain" />
              ) : (
            ...
            <nav className="hidden lg:flex items-center gap-0.5 ml-4">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="bg-secondary border-border w-52 max-h-80 overflow-y-auto">
                  {categories.length > 0 ? categories.map(c => (
                    <DropdownMenuItem key={c.slug} asChild>
                      <Link to={`/products?category=${c.slug}`} className="cursor-pointer text-sm">{c.label}</Link>
                    </DropdownMenuItem>
                  )) : (
                    <DropdownMenuItem disabled>Yüklənir...</DropdownMenuItem>
                  )}
                </DropdownMenuContent>
              </DropdownMenu>
            </nav>
            ...
            <div className="flex items-center gap-1.5 ml-auto">
              ...
              <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
                ...
                <SheetContent side="left" className="bg-secondary border-border w-[85vw] max-w-[320px] p-0 flex flex-col h-[100dvh]">
                  ...
                  <div className="p-4 border-b border-border flex flex-col gap-3 shrink-0">
                    ...
                    <Link to="/track-order" onClick={() => setMobileOpen(false)} className="flex items-center gap-1.5 text-sm font-semibold text-primary hover:underline">
                      <Package size={14} /> Sifarişi İzlə
                    </Link>
                  </div>
                  <form onSubmit={e => { handleSearch(e); setMobileOpen(false); }} className="p-4 border-b border-border shrink-0">
                  ...
                  <nav className="p-3 flex-1 flex flex-col gap-0.5 overflow-y-auto min-h-0">
                    {navLinks.map(link => (
                    ...
                    <div className="px-3 py-1 text-xs font-bold text-foreground uppercase tracking-wider">{t('nav_categories')}</div>
                    {categories.length > 0 ? categories.map(c => (
                      <Link key={c.slug} to={`/products?category=${c.slug}`} onClick={() => setMobileOpen(false)}
                        className="px-3 py-2 text-sm text-muted-foreground hover:text-foreground hover:bg-muted rounded-md transition-colors pl-5">
                        {c.label}
                      </Link>
                    )) : (
                      <div className="px-3 py-2 text-sm text-muted-foreground pl-5">Yüklənir...</div>
                    )}
                  </nav>
