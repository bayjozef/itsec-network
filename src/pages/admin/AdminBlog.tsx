...
const EMPTY: Partial<BlogPost> = {
  title: '', slug: '', excerpt: '', content: '', thumbnail_url: '', video_url: '', is_published: false,
};
      ...
      <Dialog open={dialog} onOpenChange={v => !v && setDialog(false)}>
        <DialogContent className="bg-secondary border-border max-w-[calc(100%-2rem)] md:max-w-2xl max-h-[90dvh] overflow-y-auto">
          ...
          <div className="space-y-3 py-2">
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">Video URL (YouTube/Vimeo/etc. optional)</label>
              <Input value={selected.video_url ?? ''} onChange={e => setField('video_url', e.target.value)} className="bg-muted border-border h-9 text-sm" placeholder="https://www.youtube.com/watch?v=..." />
            </div>
            <div>
